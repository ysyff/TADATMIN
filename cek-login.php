...
if ($num_rows > 0) {
    $rows = $db->db_fetch_array($result);
        ...
        $_SESSION['apriori_kopi_id'] = $rows['id']; // mengisi session
        $_SESSION['apriori_kopi_username'] = $rows['username'];
        $_SESSION['apriori_kopi_level'] = $rows['level'];

        $level_name = ($_SESSION['apriori_kopi_level']==1)?"admin":"kepala";
        $_SESSION['apriori_kopi_level_name'] = $level_name;
        $_SESSION['apriori_kopi_key'] = sha1(date("Y-m-d H:i:s") . $rows['id']);
        $_SESSION['apriori_kopi_last_login'] = date("d-m-Y H:i:s");
