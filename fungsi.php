...
function can_access_sidebar($sidebar){
    if($_SESSION['apriori_kopi_level']==2 & ($sidebar=='hasil_rule' || $sidebar=='view_rule')){
        return true;
    }
    if($_SESSION['apriori_kopi_level']==1){
        return true;
    }
    return false;
}
 ...
 * @param type $parent_sidebar headsidebar parameter get
 */
function edit_delete_link($table, $id, $parameter_key, $path_to_root, $parent_sidebar){
	start_column("align='center' ");
        edit_link($table, $id, $parameter_key, $path_to_root, $parent_sidebar);
//	echo " | ";
        delete_link($table, $id, $parameter_key, $path_to_root, $parent_sidebar);
	end_column();
}
 ...
 * @param type $parent_sidebar headsidebar parameter get
 */
function edit_link($table, $id, $parameter_key, $path_to_root, $parent_sidebar){
    echo "<a href='?".$parameter_key."kd_tabel=$table&headsidebar=$parent_sidebar&update=$id' class='btn bg-olive btn-sm margin' data-toggle='tooltip' title='Edit' >"
 ...
 */
function delete_link($table, $id, $parameter_key, $path_to_root, $parent_sidebar, $params=''){
    $prm='';
    if(!empty($params)){
        $prm = "&".$params;
    }
    echo "<a href='?".$parameter_key."kd_tabel=$table&headsidebar=$parent_sidebar&delete=".$id.$prm."' onclick=\"return confirm('Apakah anda yakin?')\" class='btn btn-danger btn-sm' data-toggle='tooltip' title='Delete'>"
